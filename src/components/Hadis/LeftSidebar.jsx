...
const LeftSidebar = async() => {
  ...
  return (
    <div className="col-start-1 hidden lg:block xl:block category-bar w-[350px] lg:max-w-full">
      <div className="rounded-2xl h-[calc(100vh_-_115px)] bg-white dark:bg-hadith-dark-card flex items-center flex-col">
        <div className="border-b-[1px] lg-max:border-y-[1px] lg-max:dark:border-y-[1px] w-full flex items-center justify-center text-lg text-white border-[#ECEEF0] dark:border-dark-cat-border">
          <p
            className="bg-hadith-button-background
              } w-1/2 text-center py-3 cursor-pointer rounded-tl-2xl text-xl style-Kalpurush-webKit"
          >
            বই
          </p>
          <p
            className="bg-white dark:bg-hadith-dark-card text-black dark:text-hadith-deepoffwhite
        ...
        <div className="w-full h-[50px] category-bar">
          <div className="relative px-4 -pr-6 my-3">
            <svg
              className="absolute inset-y-6 left-7 fill-[#5C5C5C] dark:fill-white"
              width="15"
              height="15"
              viewBox="0 0 15 15"
              fill="none"
            ...
            <input
              ...
              className="mt-2 bg-hadith-body-grey-bg dark:bg-darkchapter-list-hover w-full text-hadith-offblack p-3 rounded-lg focus:outline-0 focus:text-black dark:text-hadith-deepoffwhite pl-10 pr-10"
              type="text"
            />
          </div>
            ...
            <div className="flex gap-y-2 flex-col w-full">
              {
                data.book.map((book, i)=> {
                    return <SidebarBookCard key={i} book={book}/>
                })
              }
            </div>
          </div>
