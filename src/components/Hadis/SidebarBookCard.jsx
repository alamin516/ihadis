...
const SidebarBookCard = ({ book }) => {
  ...
  return (
    <div
      ...
      id={`bookCard_${book.book_name}`}
      name="bookCard"
      className={`p-4 flex  group cursor-pointer  false ${
        pathname === `/${book.book_name}` ? "bg-[#ebfcf6]" : ""
      } dark:bg-darkchapter-list-hover justify-between items-center rounded-2xl h-[6.25rem]`}
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col justify-between gap-1.5 group">
          ...
          <div className="text-[#40404099] dark:text-dark-text-subtitle style-Kalpurush-dropdown text-sm leading-[26px] md:text-[13px]">
            সর্বমোট হাদিস - <span>{bengaliNumeral(book.number_of_hadis)}</span>
          </div>
        </div>
      </div>
    </div>
