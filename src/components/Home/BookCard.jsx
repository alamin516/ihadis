...
const BookCard = ({ book }) => {
  ...
  return (
    <div
      onClick={() => router.replace(`/${book.book_name}`)}
      name="bookCard"
      className={`p-5 sm-max:p-4 flex  group cursor-pointer  false ${
        pathname === "/"
          ? "drop-shadow-[0_2px_20px_rgba(0,0,0,0.05)]"
          : " hover:bg-[#ebfcf6]"
      } bg-white  hover:dark:bg-darkchapter-list-hover dark:bg-hadith-dark-card justify-between items-center rounded-2xl h-[6.25rem]`}
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col justify-between gap-1.5 group">
          ...
          <div className="text-[#40404099] dark:text-dark-text-subtitle style-Kalpurush-dropdown text-sm leading-[26px] md:text-[13px]">
            হাদিসের রেঞ্জ: <span>{bengaliNumeral(book.number_of_hadis)}</span>
          </div>
        </div>
      </div>
    </div>
